...
#define HEX_MAX_SIZE         2048 // 16384
...
#define trace(fmt, args...)
#define mtrace(fmt, args...) \
    printk("[yossiku %d] <%s/%s> _MESSAGE: "fmt"\n", task_cpu(current), __SHORT_FORM_OF_FILE__, __func__, ##args)
#define ntrace(fmt, args...)
#define etrace(fmt, args...) \
    printk("[yossiku %d] <%s/%s> _ERROR: "fmt"\n", task_cpu(current), __SHORT_FORM_OF_FILE__, __func__, ##args)
                            ...
                            const char *trace_list[], const char *fmt, ...)
{
    ...
    if (trace_list) {
        ...
        while (*trace_list) {
            if (strncasecmp(func_name, *trace_list, strlen(func_name)) == 0)
                break;
            trace_list++;
        }
    ...
    len = tsprintf(kbuff, "[yossiku, %d] <%s/%s> %s\n", task_cpu(current), file_name, func_name, tbuff);
    printk(kbuff);
...
#define __ZEROPAD    1   /* pad with zero */
#define __SIGN       2   /* unsigned/signed long */
#define __PLUS       4   /* show plus */
#define __SPACE      8   /* space if plus */
#define __LEFT       16  /* left justified */
#define __SPECIAL    32  /* 0x */
#define __LARGE      64  /* use 'ABCDEF' instead of 'abcdef' */
...
static int tvsprintf(char *buf, const char *fmt, va_list args)
{
    ...
    for (str=buf ; *fmt ; ++fmt) {
        ...
        repeat:
            ...
            switch (*fmt) {
                case '-': flags |= __LEFT; goto repeat;
                case '+': flags |= __PLUS; goto repeat;
                case ' ': flags |= __SPACE; goto repeat;
                case '#': flags |= __SPECIAL; goto repeat;
                case '0': flags |= __ZEROPAD; goto repeat;
                }
